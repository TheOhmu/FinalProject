public class GradiantDecent {
	...
	public void findMinRating() {
		...
		while (min != temp) {
			...
			System.out.println("rating: " + (initialRating - change)
					+ " error: " + error1 + "DOWN");
			System.out.println("rating: " + (initialRating + change)
					+ " error: " + error2 + "UP");
	...
	public double findMinOfTwo(double change) {
		...
		while (min > temp) {
			...
			recommender.setAll(weight1 + change, weight2 - change, 0);
			error1 = tester.crossValidate(
					"c:\\data\\recommender\\allratings.txt", 5);
			...
			temp = Math.min(error1, error2);
			
			System.out.println(" error: " + error1+" weight1: "+(weight1+change)+" weight2: "+ (weight2-change));
			System.out.println("error: " + error2+" weight1: "+(weight1-change)+" weight2: "+ (weight2+change));
	...
	public double findMinOfThree(double change) {
		...
		while (min > temp) {
			...
			double e1 = error(weight1 + 2 * change, weight2 - change, weight3
					- change);
			double e2 = error(weight1 - change, weight2 + 2 * change, weight3
					- change);
			double e3 = error(weight1 - change, weight2 - change, weight3 + 2
					* change);
			System.out.println();

			// adding to 2 subtract from 1
			double e4 = error(weight1 - 2 * change, weight2 + change, weight3
					+ change);
			double e5 = error(weight1 + change, weight2 - 2 * change, weight3
					+ change);
			double e6 = error(weight1 + change, weight2 + change, weight3 - 2
					* change);
			System.out.println();
		...
		System.out.println("a:" + recommender.getWeight1() + " b:"
				+ recommender.getWeight2() + " c:" + recommender.getWeight3());
		return min;
	}

	public void set(double e1, double e2, double e3, double e4, double e5,
			double e6, double e7, double e8, double e9, double e10, double e11,
			double e12, double min, double change, double weight1,
			double weight2, double weight3) {

		if (min == e1)
			recommender.setAll(weight1 + 2 * change, weight2 - change, weight3
					- change);
		if (min == e2)
			recommender.setAll(weight1 - change, weight2 + 2 * change, weight3
					- change);
		if (min == e3)
			recommender.setAll(weight1 - change, weight2 - change, weight3 + 2
					* change);
		if (min == e4)
			recommender.setAll(weight1 - 2 * change, weight2 + change, weight3
					+ change);
		...
		if (min == e6)
			recommender.setAll(weight1 + change, weight2 + change, weight3 - 2
					* change);
		if (min == e7)
			recommender.setAll(weight1 + change, weight2 - change, weight3);
		if (min == e8)
			recommender.setAll(weight1 + change, weight2, weight3 - change);
		if (min == e9)
			recommender.setAll(weight1 - change, weight2 + change, weight3);
		if (min == e10)
			recommender.setAll(weight1, weight2 + change, weight3 - change);
		if (min == e11)
			recommender.setAll(weight1 - change, weight2, weight3 + change);
		if (min == e12)
			recommender.setAll(weight1, weight2 - change, weight3 + change);
	...
	public double minFour(double e1, double e2, double e3, double e4) {
		if (Math.min(Math.min(e1, e2), Math.min(e3, e4)) == e1)
			return e1;
		if (Math.min(Math.min(e1, e2), Math.min(e3, e4)) == e2)
			return e2;
		else if (Math.min(e3, e4) == e3)
			return e3;
		else
			return e4;
	...
	public double maxFour(double e1, double e2, double e3, double e4) {
		if (Math.max(Math.max(e1, e2), Math.max(e3, e4)) == e1)
			return e1;
		if (Math.max(Math.max(e1, e2), Math.max(e3, e4)) == e2)
			return e2;
		else if (Math.max(e3, e4) == e3)
			return e3;
		else
			return e4;
	}
	...
	public double minThree(double e1, double e2, double e3) {
		if (Math.min(Math.min(e1, e2), e3) == e3)
			return e3;
		if (Math.min(e1, e2) == e1)
			return e1;
		else
			return e2;
	}
